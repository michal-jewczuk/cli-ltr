...
impl Rerun {
    ...
    fn render_empty_items<B: Backend>(&mut self, f: &mut Frame<B>, area: Rect) {
        let text = vec![
            Spans::from(Span::raw("")),
            Spans::from(vec![
                Span::styled(t!("rerun.empty.header", locale = &self.locale), Style::default().add_modifier(Modifier::BOLD)),
            ]),
            Spans::from(Span::raw("")),
            Spans::from(Span::styled(t!("rerun.empty.info", locale = &self.locale), Style::default().add_modifier(Modifier::ITALIC))),
            Spans::from(Span::raw("")),
        ];
